...
### 5. Lọc Log ngầm tự động của SDK (Từ v1.0.7 & v1.0.8)
...
- **Tự động tiết kiệm I/O ở Production:** Nếu ứng dụng chạy ở môi trường `Production`, màn hình Console sẽ tự động thắt chặt chỉ hiển thị log từ mức `Warning` trở lên.
- **Tự động loại bỏ Healthcheck Spam:** Các log sinh ra từ việc Load Balancer ping vào các endpoint `/health`, `/ready`, `/alive` sẽ bị triệt tiêu, không bắn lên Kibana gây nhiễu.
- **Tự động lọc Log Retry (Polly):** Các dòng log rác sinh ra mỗi khi HttpClient gọi thất bại và tự động Retry (Polly `ExecutionAttempt`, `ResilienceEvent`) sẽ bị triệt tiêu hoàn toàn.
