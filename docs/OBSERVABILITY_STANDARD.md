...
3. [Quy chuẩn Đặt tên & Phân loại Dịch vụ (Resource Taxonomy)](#3-quy-chuẩn-đặt-tên--phân-loại-dịch-vụ-resource-taxonomy)
4. [Tiêu chuẩn Logging (Nhật ký Sự kiện) - Đúng, Đủ và Không Rác](#4-tiêu-chuẩn-logging-nhật-ký-sự-kiện---đúng-đủ-và-không-rác)
5. [Tiêu chuẩn Tracing (Dấu vết Phân tán) - Bám sát luồng Request](#5-tiêu-chuẩn-tracing-dấu-vết-phân-tán---bám-sát-luồng-request)
6. [Tiêu chuẩn Metrics (Số liệu Đo lường) - Xu hướng & Trạng thái](#6-tiêu-chuẩn-metrics-số-liệu-đo-lường---xu-hướng--trạng-thái)
7. [Yêu cầu Thiết kế Dashboard & Các câu hỏi Vận hành cần giải quyết](#7-yêu-cầu-thiết-kế-dashboard--các-câu-hỏi-vận-hành-cần-giải-quyết)
...
## 3. Quy chuẩn Đặt tên & Phân loại Dịch vụ (Resource Taxonomy)

Khi hệ thống mở rộng lên hàng trăm microservices thuộc nhiều dự án (Projects/Domains) khác nhau và cùng đẩy dữ liệu về một cụm OpenTelemetry Collector trung tâm, việc chuẩn hóa cách đặt tên (Naming Convention) là cực kỳ quan trọng. 

Nếu không có bộ tiêu chuẩn này, đội vận hành sẽ nhìn vào một "bể" log/trace hỗn độn, không biết service này thuộc dự án nào, team nào chịu trách nhiệm, và đang chạy ở môi trường (Environment) nào.

Dưới đây là bộ thuộc tính chuẩn (Resource Attributes) dựa trên **OpenTelemetry Semantic Conventions** bắt buộc phải được khai báo lúc khởi tạo SDK:

### 3.1. Các thuộc tính định danh lõi (Core Identity Attributes)

*   **`service.namespace` (Tên Dự án / Domain):**
    *   *Ý nghĩa:* Gom nhóm các microservices có liên quan nghiệp vụ vào một cụm lớn (ví dụ: một Gitlab Group hoặc một Domain).
    *   *Convention:* `kebab-case`, tiếng Anh, không chứa ký tự đặc biệt.
    *   *Ví dụ:* `ecommerce-core`, `payment-gateway`, `crm-system`.
*   **`service.name` (Tên Dịch vụ):**
    *   *Ý nghĩa:* Tên duy nhất của microservice hoặc ứng dụng. Tên này phải match 100% với tên Git Repository để dễ dàng tra cứu code.
    *   *Convention:* `kebab-case` kèm hậu tố **`-svc`** (bắt buộc theo quy chuẩn ISC `R-SVC-001`). Không chứa tên hạ tầng DB (như `mongo`, `sql`, `redis`).
    *   *Ví dụ:* `order-svc`, `payment-svc`, `inventory-svc`, `customer-svc`.
*   **`deployment.environment` (Môi trường triển khai):**
    *   *Ý nghĩa:* Phân tách dữ liệu giữa các môi trường để Dashboard có thể lọc ra (Filter) nhanh chóng, tránh nhầm lẫn log Dev với log Prod.
    *   *Convention:* Viết thường (lowercase). Chỉ sử dụng các giá trị chuẩn mực.
    *   *Ví dụ:* `prod`, `staging`, `uat`, `dev`.
*   **`service.version` (Phiên bản):**
    *   *Ý nghĩa:* Giúp QA/DevOps theo dõi sự biến động lỗi sau mỗi lần release.
    *   *Convention:* Sử dụng Semantic Versioning (SemVer) hoặc Git Commit Hash (Short SHA).
    *   *Ví dụ:* `1.2.0`, `v2.0.1-beta`, `a8c4f92`.

### 3.2. Cấu trúc hiển thị trên Kibana/APM (Service Map)

Khi tuân thủ nghiêm ngặt 4 thuộc tính trên, hệ thống APM (như Kibana, Jaeger, Datadog) sẽ tự động xây dựng được bản đồ kiến trúc (Service Map) cực kỳ rõ ràng:

1.  **Level 1 (Filtering):** Môi trường `deployment.environment = prod`
2.  **Level 2 (Grouping):** Nhóm dự án `service.namespace = ecommerce-core`
3.  **Level 3 (Nodes):** Các dịch vụ liên kết với nhau `service.name = order-svc` gọi sang `inventory-svc`.

**Lợi ích đối với vận hành:**
*   **Developer:** Bị lỗi ở `order-svc` -> Lên Kibana search `service.name: "order-svc" AND deployment.environment: "prod"` -> Lấy mã lỗi đối chiếu vào Git Repo `order-svc` để fix.
*   **DevOps/SA:** Nhìn vào Service Map, phát hiện ngay nhóm `ecommerce-core` đang gọi chéo (cross-domain) quá nhiều sang nhóm `crm-system`, gây nguy cơ thắt cổ chai kiến trúc (Architectural Bottleneck), từ đó có kế hoạch refactor.

---

## 4. Tiêu chuẩn Logging (Nhật ký Sự kiện) - Đúng, Đủ và Không Rác
...
### 4.1. Ghi Log có cấu trúc (Structured Logging) - Bắt buộc
...
### 4.2. Tiêu chuẩn phân định Log Levels
...
### 4.3. Quy tắc vàng để Tránh "Log Rác" (Log Hygiene)
...
3.  **Tắt log mặc định của Thư viện bên thứ ba (Framework/Vendor Logs):**
    *   Mặc định, các framework như ASP.NET Core, EF Core, HttpClient, Hangfire... sinh ra rất nhiều log thừa ở mức `INFO` (kết nối DB, câu query SQL, thông tin route) gây nhiễu Kibana.
    *   *Cấu hình bắt buộc:* Từ **SDK v1.0.5**, thêm section `Serilog` vào `appsettings.json` để đè (Override) mức log của các vendor này lên `Warning` hoặc `Error`:
        ```json
        "Serilog": {
          "MinimumLevel": {
            "Default": "Information",
...
## 5. Tiêu chuẩn Tracing (Dấu vết Phân tán) - Bám sát luồng Request
...
### 5.1. Trực Quan Hóa Distributed Tracing (Multi-Instrument)
...
### 5.2. Truyền dẫn Ngữ cảnh (Context Propagation)
*   **Chuẩn W3C Trace Context & B3 Service Mesh:** Bắt buộc sử dụng chuẩn W3C (`traceparent` header: `00-{trace_id}-{span_id}-{trace_flags}`). 
    *   *Service Mesh Support:* Đối với các cụm chạy qua Istio / Envoy Proxy sử dụng header B3 (`x-b3-traceid`), SDK đã tích hợp sẵn `CompositeTextMapPropagator` hỗ trợ song song đồng thời **W3C, B3 (Single & Multi), và Baggage** để không bị đứt đoạn phân vết xuyên qua Gateway/Sidecar.
...
### 5.3. Khi nào cần tạo Custom Span?
...
### 5.4. Quy tắc gắn Tag (Attributes) trên Spans
...
## 6. Tiêu chuẩn Metrics (Số liệu Đo lường) - Xu hướng & Trạng thái
...
### 6.1. Hai mô hình đo lường cốt lõi
...
### 6.2. Lựa chọn đúng loại Metric Instrument
...
### 6.3. Cảnh báo về Bùng nổ Nhãn (Cardinality Explosion)
...
## 7. Yêu cầu Thiết kế Dashboard & Các câu hỏi Vận hành cần giải quyết
...
### CẤP ĐỘ 4: QA COMPLIANCE TRACKER (TIÊU CHÍ BẮT BUỘC CHO QUALITY GATE 2)
*   **Đối tượng phục vụ:** QA, Tech Lead, PM, Ban Quản lý Chất lượng.
*   **Mục tiêu:** Kiểm tra và bảo đảm 100% microservice trước khi phát hành lên Production đã tích hợp đầy đủ SDK chuẩn hóa và đang phát tín hiệu telemetry hợp lệ.

#### Các thành phần hiển thị bắt buộc:
1.  **SDK Adoption & Active Status (Stat / Gauge):**
    *   Theo dõi metric `observability.sdk.active = 1` của từng service do SDK tự động phát ra. Nếu metric này bằng 0 hoặc missing, Quality Gate 2 sẽ tự động BLOCK không cho phép release.
2.  **Telemetry Pillar Completeness (Table / Matrix):**
    *   Bảng ma trận kiểm tra từng service có đủ cả 3 trụ cột (Logs streaming, Traces flowing, Metrics reporting).
3.  **Trace Context Propagation Ratio (Gauge):**
    *   Tỷ lệ phần trăm các request xuyên service bảo toàn được `TraceId` mà không bị đứt đoạn qua Gateway/Service Mesh (Target: 100%).

#### Câu hỏi vận hành Dashboard 4 phải trả lời được:
*   *Câu hỏi:* **Service này đã đủ điều kiện đạt Quality Gate 2 và sẵn sàng Go-live hay chưa?**
    *   *Cách trả lời:* Nhìn vào chỉ số `observability.sdk.active` và trạng thái 3 trụ cột trên bảng ma trận tuân thủ QA.
