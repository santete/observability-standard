...
*   **`service.name` (Tên Dịch vụ):**
    ...
    *   *Convention:* `kebab-case`. Không bao gồm hậu tố môi trường (dev/prod) vào tên service.
    *   *Ví dụ:* `order-processor`, `user-auth-api`, `inventory-worker`.
...
3.  **Level 3 (Nodes):** Các dịch vụ liên kết với nhau `service.name = order-processor` gọi sang `inventory-worker`.

**Lợi ích đối với vận hành:**
*   **Developer:** Bị lỗi ở `order-processor` -> Lên Kibana search `service.name: "order-processor" AND deployment.environment: "prod"` -> Lấy mã lỗi đối chiếu vào Git Repo `order-processor` để fix.
