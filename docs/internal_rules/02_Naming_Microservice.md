...
## R-OBS: Observability conventions (Log, Trace, Metrics Attributes)

### R-OBS-FIELD-001
- **title**: Observability log attribute naming
- **severity**: REQUIRED
- **tier**: DEV_SELF_CHECK
- **rule**: Mọi custom attribute / property trong Log, Trace và Metrics PHẢI dùng `snake_case`, tuân theo OpenTelemetry Semantic Conventions.
- **pattern**: `^[a-z][a-z0-9_]*$`
- **examples_pass**:
  - `order_id`
  - `customer_name`
  ...
  - `machine_name`
- **examples_fail**:
  - `OrderId` — không được PascalCase (mặc định Serilog)
  - `orderId` — không được camelCase
  - `CorrelationId` — không được PascalCase
- **rationale**: Đồng bộ với OTel Semantic Conventions (`service.name`, `http.status_code`) và với API response convention (R-RESP-FIELD-001). Giảm cognitive load cho Dev khi chỉ cần nhớ 1 quy tắc cho data output. SDK đảm bảo tương thích ngược bằng cách cung cấp song song cả alias PascalCase cho các hệ thống cũ.
...
| Observability Attribute | `snake_case` (vd: `order_id`, `correlation_id`) (như nhau) |
